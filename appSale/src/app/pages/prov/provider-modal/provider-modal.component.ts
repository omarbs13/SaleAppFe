...
export class ProviderModalComponent implements OnInit {
  ...
  constructor(
    ...
    if (!this.isNewProvider) {
      this.providerService.geProvider(data.id).subscribe((res) => {
        this.getCities(this.data.stateId);
  ...
  onSubmit() {
    ...
    if (!this.isNewProvider) {
      this.providerService
        .putProvider(this.providerForm.value)
        .subscribe((data: any) => {
          if (data.success) {
            this.data.success = true;
            this.dialogRef.close(this.data);
  ...
  createForm(provider?: ProviderModel) {
    ...
    this.providerForm = new FormGroup({
      ...
      email: new FormControl(provider?.email, [
        Validators.required,
        Validators.email,
      ]),
      rfc: new FormControl(provider?.rfc, [
        Validators.required,
        Validators.pattern(RfcValid),
      ]),
      street: new FormControl(provider?.street, Validators.required),
      stateId: new FormControl(provider?.stateId, Validators.required),
      state: new FormControl(''),
      cityId: new FormControl(provider?.cityId),
      cityName: new FormControl(''),
